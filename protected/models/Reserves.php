...
class Reserves extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, description, quantity, last_update, is_full', 'required'),
			array('quantity, is_full', 'numerical', 'integerOnly'=>true),
			array('name, description', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, description, quantity, last_update, is_full', 'safe', 'on'=>'search'),
		);
	}
